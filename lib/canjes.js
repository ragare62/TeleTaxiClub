...
module.exports.confirmaCarroCopia = function(idMiembro, callback) {
    var html = "";
    var email = "";
    var nombre = "";
    var lC = "";
...
module.exports.confirmaCarro = function(idMiembro, callback) {
    ...
    var lC = "";
    if (!idMiembro) {
        var err = new Error('Debe indicar el miembro del que debe confirmar el carro');
        callback(err);
        return;
    }
    getCarroMiembroInt(idMiembro, function(err, carro) {
        ...
        // reformateamos las fechas para que queden bien
        for (var i = 0; i < carro.length; i++) {
            carro[i].fecha = moment(carro[i].fecha).format('DD/MM/YYYY');
        }
        var c = carro[0];
        var data2 = {
            email: c.miembro.email,
            nombre: c.miembro.nombre + " " + c.miembro.primerApellido,
            canjes: carro
        }
        fs.readFile('./plantillas/plt_canje_carro.html', function(err, data) {
            if (err) throw err;
...
function getCarroMiembroInt(id, callback) {
    ...
    var canjes = [];
    // comprobamos si nos han pasado la id
    sql = 'SELECT c.idCanje, m.idMiembro, m.nombre, m.primerApellido, m.segundoApellido, m.email, c.fecha, p.idPremio, p.titulo, p.textoCanje, c.unidades, c.puntos';
    sql += ' FROM canjes AS c';
    sql += ' LEFT JOIN miembro AS m ON m.idMiembro = c.idMiembro';
    sql += ' LEFT JOIN premio AS p ON p.idPremio = c.idPremio';
    sql += ' WHERE c.enCarro';
